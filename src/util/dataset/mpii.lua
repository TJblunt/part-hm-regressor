...
function Dataset:__init()
    ...
    local a = hdf5.open(paths.concat('/home/jtang/pose_estimation/pose-hg-train/','data/mpii/annot.h5'),'r')
    ...
    if not opt.idxRef then
        ...
        if not opt.randomValid then
            ...
            local validAnnot = hdf5.open(paths.concat('/home/jtang/pose_estimation/pose-hg-train/', 'data/mpii/annot/valid.h5'),'r')
...
function Dataset:getPath(idx)
    return paths.concat('/home/jtang/pose_estimation/pose-hg-train/data/mpii/','images',ffi.string(self.annot.imgname[idx]:char():data()))
end
