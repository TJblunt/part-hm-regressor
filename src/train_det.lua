...
function step(tag)
    ...
    for i,sample in loader[set]:run() do
        ...
        --local output_log = applyFn(function (x) return x:clone() end, output)
        --output_log = applyFn(function (x) return x:apply(function(y) return math.log(math.exp(y)/(math.exp(y)+math.exp(1-y))) end) end, output_log)
        --output_log = applyFn(function (x) return x:apply(function(y) return math.log(y) end) end, output_log)
